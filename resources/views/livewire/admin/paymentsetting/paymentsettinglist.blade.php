                ...
                <div class="ms-auto text-end">
                   <div class="row">
                      ...
                      <form wire:submit.prevent="applyFilter" >
                        <div class="page-breadcrumb d-none d-sm-flex align-items-center py-3" style="background:#fff">
                            
                           <div class="px-2">
                              <select wire:model="selectedtitle" class="resizeselect filt1 form-control form-select" style="width:150px !important">
                          ...
                          <div class="px-2">
                           <input wire:model="search" type="text" name="search" class="form-control filt3" placeholder="Start Typing To Search">
                          </div>
                        <div class="px-2">
                        ...
                        @if($filterApplied)
                            <div class="px-2">
                                <button type="button" wire:click="clearFilter" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">Clear</button>
                            </div>
                        @endif
                    </div>
                    </form>
                      ...
                      <table id="dataTable" class="table table-striped table-bordered">
                         ...
                         <thead>
                            <tr>
                               ...
                               <th class="sort" wire:click="columnSortOrder('title')">Title {!! $sortLink !!}
                               </th>
                               <th class="sort" wire:click="columnSortOrder('slug')">Slug {!! $sortLink !!}
                               </th>
                               <th class="sort" wire:click="columnSortOrder('cc_email')">CC Email {!! $sortLink !!}
                               </th>
                               <th class="sort" wire:click="columnSortOrder('bcc_email')">BCC Email {!! $sortLink !!}
                               </th>
                         ...
                         <tbody>
                            ...
                            <tr>
                               ...
                               <td>
                                 <button style="font-size: 12px;" wire:click="status_update({{ $Paymentsetting->id }})" class="btn btn-info btn-sm" onclick="return confirm('Are you sure you want to change this Paymentsetting status?');">
                                    @if($Paymentsetting->status == 0) Inactive @else Active @endif
                                </button>
                               </td>
                               <td>
                                  <a style="font-size: 8px;padding-left: 5px; padding-right: 0px;" href="{{ route('admin.paymentsettings.paymentsettingview', ['id' => $Paymentsetting->id]) }}"  wire:navigate class="btn btn-secondary"
                                    wire:navigate class="btn btn-secondary"
                                    title="Edit {{ $heading }}"><i class="fa fa-eye"></i></a>   
                                  <a style="font-size: 8px;padding-left: 5px; padding-right: 0px;" href="{{ route('admin.paymentsettings.paymentsettingedit', ['paymentsettings' => $Paymentsetting->id]) }}"
                                     wire:navigate class="btn btn-secondary"
