  ...
  --}}
                    ...
                    <div class="table-responsive">
                        <table id="dataTable" class="table table-striped table-bordered">
                            ...
                            <!-- Table body -->
                            <tbody>
                                @if ($schoolprogram->count())
                                    @foreach ($schoolprogram as $schoolprograms)
                                        <tr>
                                            <td>{{ $loop->iteration}}</td>
                                            <td>{{ $schoolprograms->name }} </td>
                                            <td>
                                               <button style="font-size: 8px;padding-left: 5px; padding-right: 0px;" 
                                                class="btn btn-secondary"
                                                  
                                            wire:click="edit({{ $schoolprograms->id }})"  title="Edit {{ $heading }}"><i class="bx bx-edit"></i></button>
 
 
                                                <button style="font-size: 8px;padding-left: 5px; padding-right: 0px;" class="btn  bg-red-500 text-white" type="button"
                                                    wire:click="delete({{ $schoolprograms->id }})"
                 ...
                 <div class="fixed inset-0 flex items-center justify-center z-50">
                    ...
                    <div class="relative bg-gray-200 p-8 rounded shadow-lg w-1/2">
                       ...
                       <path d="M14.53 4.53l-1.06-1.06L9 7.94 4.53 3.47 3.47 4.53 7.94 9l-4.47 4.47 1.06 1.06L9 10.06l4.47 4.47 1.06-1.06L10.06 9z" />
                        ...
                        <h2 class="text-2xl font-bold mb-4">{{ $spId ? 'Edit' : 'Create' }} {{$heading}}</h2>
                        <form  wire:submit.prevent="{{ $spId ? 'update' : 'store' }}">
                            <div class="mb-4">
