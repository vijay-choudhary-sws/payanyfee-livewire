        ...
        <div class="col-12">
            ...
            @if ($in_data->inputType->type == 'checkbox')
                @foreach ($in_data->metaOption as $key => $item)
                    ...
                    <input type="checkbox"  class="form-check-input" name="{{ $in_data->input_name }}" id="{{ $in_data->inputType->tag_name }}{{ $in_data->id }}{{$key}}" {{$item->is_default == '1' ? 'checked' : '' }}>
            ...
            @elseif ($in_data->inputType->type == 'radio')
                ...
                @endforeach
            @else
                <input  type="{{ $in_data->inputType->type }}" class="form-control" name="{{ $in_data->input_name }}"
