...
class PaymentGetWay extends Component
{
    ...
    #[Rule('required')]
    public $status;
    public $isOpen = 0, $isedit = 0, $paymodeedit = false;
    public $pgwId;
    public $search;
    public $selectedtitle,$id;
    public $filterApplied = false;
    public $gatewayCreat = false;
    public $clearfilter = "";
    public $paymentMode;
    public $amount = [], $percent = [], $modestatus = [];
    public $modelContent, $modelTitle = 'hey';


    public function mount()
    {
        // $this->paymentgateways = Paymentgetways::paginate(10);
    ...
    {
        $this->gatewayCreat = false;
        $this->isOpen = false;
        $this->paymodeedit = false;
        $this->amount = $this->percent = $this->modestatus = [];
    }
    ...
    public function store()
    {
        ...
        try {
            ...
            $this->closeModal();
            $this->reset('name', 'selectpaymentcountry', 'photo', 'status', 'amount', 'percent', 'modestatus');
    ...
    {
        $gateway = Paymentgetways::with('gatewayMeta')->findOrFail($id);
        $this->id = $id;
        foreach ($this->paymentMode as $paymode) {
            foreach($gateway->gatewayMeta as $metaMode){
                if($metaMode->payment_mode_id == $paymode->id){
                    $this->amount[$paymode->id] = $metaMode->amount;
                    $this->percent[$paymode->id] = $metaMode->is_percent == 1 ? true : false;
                    $this->modestatus[$paymode->id] = $metaMode->status  == 1 ? true : false;
                }
            }
        }

        $this->paymodeedit = true;
        $this->openModal();
    }
    ...
    public function editPaymentMode()
    {
        ...
        foreach ($this->paymentMode as $mode) {
            ...
            if (array_key_exists($mode->id, $this->amount)) {
                $amount = $this->amount[$mode->id]? $this->amount[$mode->id] : 0;
            };
            ...
            GatewayMeta::updateOrInsert(
                ['getway_id' => $this->id, 'payment_mode_id' => $mode->id],
                ['amount' => $amount,'is_percent' => $percent,'status' => $modestatus,]);
